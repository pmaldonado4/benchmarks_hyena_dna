...
from pathlib import Path  # (Used only for string operations)
import json

class GenomicBenchmarkDataset(Dataset):
    '''
    This dataset reads genomic benchmark files directly from an Amazon S3 bucket.
    It assumes that data is organized in S3 with keys of the form:
       dataset_name/split/label_type/filename
    '''
    def __init__(
        self,
        split,
        ...
        # List S3 objects under the prefix: dataset_name/split/
        prefix = f"{dataset_name}/{split}/"
        ...
        for page in page_iterator:
            for obj in page.get('Contents', []):
                key = obj['Key']
                # Assume key format: dataset_name/split/label_type/filename
                parts = key.split('/')
                if len(parts) >= 3:
                    label_type = parts[2]
                    if label_type not in label_mapper:
                        label_mapper[label_type] = len(label_mapper)
                    self.all_keys.append(key)
                    self.all_labels.append(label_mapper[label_type])
        
        print(f"Found {len(self.all_keys)} files in S3 for dataset {dataset_name} split {split}.")
    ...
    def __getitem__(self, idx):
        ...
        # If the file is empty, return a default padded sequence
        if not content:
            default_seq = [self.tokenizer.pad_token_id] * self.max_length
            return torch.LongTensor(default_seq), torch.LongTensor([self.all_labels[idx]])
...
def train(model, device, train_loader, optimizer, epoch, loss_fn, log_file, log_interval=10):
    ...
    log_results(log_file, {"epoch": epoch, "phase": "train", "loss": avg_loss})
...
def test(model, device, test_loader, loss_fn, log_file, epoch, best_metrics):
    ...
    log_results(log_file, {"epoch": epoch, "phase": "test", "loss": test_loss, "accuracy": accuracy})

    # Update the best-performing model if applicable
...
def run_train(dataset_name, model_type):
    # Define median sequence lengths for each dataset
    median_lengths = {
        'dummy_mouse_enhancers_ensembl': 2400,
        'demo_coding_vs_intergenomic_seqs': 200,
        'demo_human_or_worm': 200,
        'human_enhancers_cohn': 500,
        'human_enhancers_ensembl': 300,
        'human_ensembl_regulatory': 500,
        'human_nontata_promoters': 300,
        'human_ocr_ensembl': 400,
    }
    ...
    print(f"Running experiment on dataset: {dataset_name} with model type: {model_type}")
    ...
    # Create the tokenizer (adjust as needed)
    ...
    log_results(log_file, {
        ...
        "num_test_samples": len(ds_test)
    })

    for epoch in range(num_epochs):
        train(model, device, train_loader, optimizer, epoch, loss_fn, log_file)
        test(model, device, test_loader, loss_fn, log_file, epoch, best_metrics)

    print(f"Best accuracy for dataset '{dataset_name}' with model '{model_type}': {best_metrics['accuracy']:.2f}% at loss {best_metrics['loss']:.4f}")
    log_best_performance_csv(dataset_name, model_type, best_metrics['accuracy'], best_metrics['loss'])
def run_all_experiments():
    # List of dataset names to download and then use for training.
    dataset_names = [
    ...
    # -----------------------------------------------
    # Now, start training experiments for each dataset and model type.
    # -----------------------------------------------
