...
def save_metrics_to_csv(dataset_name, model_name, accuracy, loss, epoch, timestamp):
    ...
    with open(csv_file, 'a', newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(['Dataset', 'Model', 'Accuracy', 'Loss', 'Epoch', 'Timestamp'])
        writer.writerow([dataset_name, model_name, accuracy, loss, epoch, timestamp])
...
def run_train():
    ...
    for pretrained_model_name in pretrained_models:
        print(f"\n{'='*50}")
        print(f"Training with model: {pretrained_model_name}")
        print(f"{'='*50}")
        ...
        # Verify model weights are different
        print(f"\nModel verification:")
        print(f"Model name: {model.__class__.__name__}")
        print(f"First layer weight mean: {model.backbone.embeddings.word_embeddings.weight.mean().item():.4f}")
        print(f"First layer weight std: {model.backbone.embeddings.word_embeddings.weight.std().item():.4f}")
        ...
        for epoch in range(num_epochs):
            ...
            # Update best metrics if we have better performance
            if accuracy > best_accuracy:
                best_accuracy = accuracy
                best_loss = test_loss
                best_epoch = epoch
                
                print(f"\nNew best performance for {pretrained_model_name}:")
                print(f"Accuracy: {best_accuracy:.2f}%")
                print(f"Loss: {best_loss:.4f}")
                print(f"Epoch: {best_epoch}")
        
        print(f"\nTraining completed for {pretrained_model_name}")
        print(f"Best accuracy: {best_accuracy:.2f}%")
        print(f"Best loss: {best_loss:.4f}")
        print(f"Best epoch: {best_epoch}")
        
        # Save only the best overall metrics for this model
