...
class HyenaDNAPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(cls,
        ...
        # Update the sequence length in the config to match the pretrained model
        if 'layer' in config and 'l_max' in config['layer']:
            config['layer']['l_max'] = 1026  # Set to match the pretrained model's sequence length
...
import csv
import random
# Import the dataset download/check functions from your genomic_benchmarks package.
from genomic_benchmarks.loc2seq import download_dataset
from genomic_benchmarks.data_check import is_downloaded
...
# ------------------------------------------------------------------------------
# Local Dataset Download Destination
# ------------------------------------------------------------------------------
...
# Assuming the following helper functions and classes are defined elsewhere:
# - is_downloaded, download_dataset, coin_flip, string_reverse_complement
# - HyenaDNAPreTrainedModel, HyenaDNAModel, CharacterTokenizer
...
class GenomicBenchmarkDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for x in base_path.iterdir():
            if x.is_dir():
                label_mapper[x.stem] = len(label_mapper)

        # Validate number of classes matches d_output
        if len(label_mapper) != d_output:
            raise ValueError(f"Dataset {dataset_name} has {len(label_mapper)} classes but d_output is set to {d_output}")

        # Load all samples and labels
        for label_type, label_idx in label_mapper.items():
            label_path = base_path / label_type
        ...
        if not self.all_paths:
            raise ValueError(f"No samples found in {base_path}")

        # Print dataset statistics
        print(f"Loaded {len(self.all_paths)} samples with {len(label_mapper)} classes")
        for label_type, label_idx in label_mapper.items():
            count = sum(1 for l in self.all_labels if l == label_idx)
            print(f"Class {label_type} (idx {label_idx}): {count} samples")
    ...
    def __getitem__(self, idx):
        try:
            ...
            if target.max() >= self.d_output:
                raise ValueError(f"Target label {target.item()} is out of bounds for {self.d_output} classes")

            return seq, target
        except Exception as e:
            print(f"Error loading sample {idx} from {txt_path}: {str(e)}")
            # Return a dummy sample instead of raising to avoid batch failures
...
# Logging function (kept for additional logging if needed)
# def log_results(log_file, data):
#     with open(log_file, "a") as f:
#         f.write(json.dumps(data) + "\n")

# New helper function to log best performance to a CSV file
def log_best_performance_csv(dataset_name, model_type, best_accuracy, best_loss, csv_file='best_performance.csv'):
    file_exists = os.path.isfile(csv_file)
    with open(csv_file, 'a', newline='') as csvfile:
         fieldnames = ['dataset', 'model_type', 'best_accuracy', 'best_loss']
...
def train(model, device, train_loader, optimizer, epoch, loss_fn, log_file, log_interval=10):
    model.train()
    total_loss = 0
    correct = 0
    total_samples = 0
    ...
    try:
        for batch_idx, (data, target) in enumerate(train_loader):
            try:
                ...
                if batch_idx == 0:
                    print(f"Batch shapes - data: {data.shape}, target: {target.shape}")
                ...
                if batch_idx % log_interval == 0:
                    print(f'Epoch {epoch} [{batch_idx * len(data)}/{len(train_loader.dataset)} ({100. * batch_idx / len(train_loader):.0f}%)] Loss: {loss.item():.6f} Acc: {100. * correct / total_samples:.2f}%')
            except Exception as e:
        ...
        if total_samples == 0:
            raise ValueError("No samples were processed in this epoch. Check data loading and batch processing.")
...
def test(model, device, test_loader, loss_fn, log_file, epoch, best_metrics):
    model.eval()
    test_loss = 0
    correct = 0
    total_samples = 0
    ...
    try:
        ...
        if accuracy > best_metrics["accuracy"]:
            ...
            torch.save(model.state_dict(), "best_performing.pt")
...
# Modified run_train function to accept parameters for dataset and model type,
# and adjust max_length based on the dataset's median sequence length.
def run_train(dataset_name, model_type):
    try:
        print(f"\nStarting training for {dataset_name} with {model_type}")
        
        median_lengths = {
            'dummy_mouse_enhancers_ensembl': 1024,
            'demo_coding_vs_intergenomic_seqs': 200,
            'demo_human_or_worm': 200,
            'human_enhancers_cohn': 500,
            'human_enhancers_ensembl': 269,
            'human_ensembl_regulatory': 401,
            'human_nontata_promoters': 251,
            'human_ocr_ensembl': 315,
        }
        
        dataset_classes = {
            'dummy_mouse_enhancers_ensembl': 2,
            'demo_coding_vs_intergenomic_seqs': 2,
            'demo_human_or_worm': 2,
            'human_enhancers_cohn': 2,
            'human_enhancers_ensembl': 2,
            'human_ensembl_regulatory': 3,
            'human_nontata_promoters': 2,
            'human_ocr_ensembl': 2,
        }
        
        max_length = median_lengths.get(dataset_name, 200)
        n_classes = dataset_classes.get(dataset_name, 2)
        ...
        log_file = f"training_log_{dataset_name}_{model_type}.json"
        best_metrics = {"accuracy": 0, "loss": float("inf")}
        ...
        elif model_type == 'metabolic':
            pretrained_model_name = 'hyenadna-metabolic-1k'
        else:
            raise ValueError("Invalid model_type. Choose 'standard' or 'metabolic'.")
        ...
        if pretrained_model_name in ['hyenadna-tiny-1k-seqlen']:
            ...
            config_path = '/projects/bdhi/benchmarks_hyena_dna/checkpoints/hyenadna-metabolic-1k/config.json'
            with open(config_path, 'r') as f:
                backbone_cfg = json.load(f)
            if 'layer' in backbone_cfg and 'l_max' in backbone_cfg['layer']:
                backbone_cfg['layer']['l_max'] = max_length + 2
            model = HyenaDNAModel(**(backbone_cfg or {}), use_head=use_head, n_classes=n_classes)

        # Add dropout to the model
        if hasattr(model, 'backbone'):
            for layer in model.backbone.layers:
                if hasattr(layer, 'dropout1'):
                    layer.dropout1.p = 0.1
                if hasattr(layer, 'dropout2'):
                    layer.dropout2.p = 0.1
        ...
        for epoch in range(num_epochs):
            train(model, device, train_loader, optimizer, epoch, loss_fn, log_file)
            test_acc, test_loss = test(model, device, test_loader, loss_fn, log_file, epoch, best_metrics)
            
            # Update learning rate based on test accuracy
            scheduler.step(test_acc)
            
            # Early stopping check
            if test_acc > best_metrics["accuracy"] + min_delta:
                best_metrics["accuracy"] = test_acc
                best_metrics["loss"] = test_loss
                best_epoch = epoch
                early_stopping_counter = 0
                # Save best model
                torch.save(model.state_dict(), f"best_model_{dataset_name}_{model_type}.pt")
            else:
        ...
        print(f"\nTraining completed! Best accuracy: {best_metrics['accuracy']:.2f}% at epoch {best_epoch + 1}")
        log_best_performance_csv(dataset_name, model_type, best_metrics['accuracy'], best_metrics['loss'])
        
    except Exception as e:
        print(f"Error during training: {str(e)}")
        raise e

# New function to loop over datasets and model types
def run_all_experiments():
    ...
    model_types = ['standard', 'metabolic']
