...
from src.dataloaders.datasets.multispecies_dataset import MultispeciesDataset
...
"""
class Species(SequenceDataset):
    """
    Dataset class for handling multiple species genomes.
    Inherits the base SequenceDataset class.
    """

    ###### very important to set this! ######
    _name_ = "multispecies"  # This name is how the dataset config finds the right dataloader
    #########################################

    def __init__(self, species, species_dir, bed_files, fasta_files, tokenizer_name=None, dataset_config_name=None,
                 d_output=None, max_length=1024, rc_aug=False, max_length_val=None, max_length_test=None,
                 val_ratio=0.0005, val_split_seed=2357, add_eos=True, detokenize=False, val_only=False,
                 batch_size=32, batch_size_eval=None, num_workers=1, shuffle=False, pin_memory=False,
                 drop_last=False, fault_tolerant=False, ddp=False, fast_forward_epochs=None,
                 fast_forward_batches=None,*args, **kwargs):
        """
        Args:
            species (list): List of species names.
            species_dir (str): Directory containing species genomes.
            bed_files (dict): Dictionary mapping species to their bed files.
            fasta_files (dict): Dictionary mapping species to their fasta files.
            tokenizer_name (str): Name of tokenizer (either 'char' or 'bpe').
            max_length (int): Maximum sequence length.
            batch_size (int): Batch size for dataloaders.
            shuffle (bool): Whether to shuffle training data.
            num_workers (int): Number of workers for DataLoader.
            fault_tolerant (bool): Use fault-tolerant dataloading (for DDP).
            ddp (bool): Distributed data parallel flag.
            **kwargs: Additional arguments.
        """
        super().__init__(*args, **kwargs)  # Call parent constructor if needed for base functionality
        self.species = species
        self.species_dir = species_dir
        self.bed_files = bed_files
        self.fasta_files = fasta_files
        self.tokenizer_name = tokenizer_name
        self.d_output = d_output if d_output is not None else len(species)
    ...
    def setup(self, stage=None):
        ...
        if self.tokenizer_name == 'char':
            ...
            self.tokenizer = CharacterTokenizer(
                ...
                model_max_length=self.max_length + 2,  # Add 2 since default adds SOS/EOS tokens, crop later
    ...
    def init_datasets(self):
        """Initialize SpeciesDataset instances for train, validation, and test splits."""
    ...
    def train_dataloader(self, *args: Any, **kwargs: Any) -> DataLoader:
        ...
        print(f"Dataset being used in train_dataloader: {type(self.dataset_train)}", flush=True)
    ...
    def on_train_dataloader_start(self):
        """Hook called when the train dataloader starts"""
        print(f"Dataset being used in train_dataloader: {type(self.dataset_train)}")
