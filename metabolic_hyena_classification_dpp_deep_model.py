...
#     # SLURM_LOCALID is often set to the local rank
#     os.environ['LOCAL_RANK'] = os.environ.get('SLURM_LOCALID', '0')
#     print("Distributed training initialized with:")
#     print("RANK:", os.environ['RANK'])
#     print("WORLD_SIZE:", os.environ['WORLD_SIZE'])
#     print("LOCAL_RANK:", os.environ['LOCAL_RANK'])

# Suppress CUDA allocator warning
import warnings
...
def setup_distributed():
    """Initialize distributed training and print diagnostic information."""
...
# --Backbone model (stack of blocks)--
...
class SequenceDecoder(nn.Module):
    def __init__(self, d_model, d_output=None, l_output=1, use_lengths=False, mode="pool",
                 hidden_dims=[2048, 1024, 512, 256], dropout=0.2, residual=True):
        super().__init__()
        ...
        self.squeeze = False  # Initialize squeeze parameter
    ...
    def forward(self, x, state=None, lengths=None, l_output=None):
        ...
        if self.mode == "last":
            ...
            def restrict(x):
                L = x.size(-2)
                if l_output == 1:
                    # For l_output=1, simply take the mean of the sequence
                    return x.mean(dim=-2, keepdim=True)
                
                if l_output > L:
                    # If l_output is greater than sequence length, pad with zeros
                    pad_size = l_output - L
        ...
        # For BatchNorm1d, we need to reshape if we have a 3D tensor
        if len(x.shape) == 3:
            b, l, d = x.shape
            x = x.reshape(-1, d)
...
class ResidualAdd(nn.Module):
    ...
    def forward(self, x):
        return x + x.clone()  # Clone to avoid in-place operation
...
class HyenaDNAModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_layer: int, d_inner: int, vocab_size: int,
        ...
        if self.use_head:
            # For binary classification, we need to ensure the output layer is configured correctly
            self.head = SequenceDecoder(d_model=d_model, d_output=d_model, l_output=1, mode='pool')
            # Add a final linear layer to ensure output shape is correct
    ...
    def forward(self, input_ids, position_ids=None, state=None): # state for the repo interface
        ...
        if self.use_head:
            ...
            x = x.squeeze(-2)  # Remove the sequence length dimension
            return self.final_layer(x)
        else:
            return hidden_states

# --Huggingface Pretrained Wrapper--
...
file_key = "training_data/full_dataset_sequences_cleaned.csv"
...
def read_training_data_from_s3(bucket_name, file_key):
    """
    Read training data CSV file from S3 bucket and return as pandas DataFrame.
    ...
    Returns:
        pandas.DataFrame: DataFrame containing the sequences and their metabolic states
    """
    try:
        print(f"Fetching training data from S3: {file_key}")
        # Get object from S3
        ...
        print(f"Error reading training data from S3: {e}")
        return None


# --Data Loading--
df = read_training_data_from_s3(
    bucket_name="metabolic-atac-peaks",
    file_key=file_key
)

# Map metabolic states to binary classification
df['metabolic_state'] = df['metabolic_state'].map({
    'fed': 'sensitive',
    'fasted': 'sensitive', 
    'refed': 'sensitive',
...
n_classes = 2  # Now we only have sensitive vs insensitive

# --Test Dataset--
#df = df.sample(n=1000, random_state=seed) # used for testing code

# --MetabolicDataset Class--

class MetabolicDataset(torch.utils.data.Dataset):
    """Dataset for metabolic state classification from DNA sequences
    Input file is a pandas DF with columns called sequence, containing DNA sequences (ACTGN) and
    metabolic_state column which contains the respective label for that sequence eg fasted, fed etc.
    """
    
    def __init__(
        self,
        df,  # pandas dataframe with 'sequence' and 'metabolic_state' columns
        max_length,
        tokenizer,
        use_padding=True,
        ...
        # Create label encoder for metabolic states
        self.label_encoder = {state: idx for idx, state in enumerate(df['metabolic_state'].unique())}
        self.n_classes = len(self.label_encoder)
    ...
    def __getitem__(self, idx):
        ...
        label = self.label_encoder[self.df.iloc[idx]['metabolic_state']]
...
# --- Distributed setup ---
def setup_distributed():
    # Assume the environment variables are set (e.g., RANK, WORLD_SIZE) (use torch_run)
    dist.init_process_group(backend='nccl')
...
# --- Freeze early layers ---
def freeze_early_layers(model, freeze_up_to=1):
    """
    Freeze the first 'freeze_up_to' blocks in the backbone.
    Adjust freeze_up_to as needed.
    """
    for i, layer in enumerate(model.backbone.layers):
        if i < freeze_up_to:
            for param in layer.parameters():
                param.requires_grad = False
    print(f"Froze first {freeze_up_to} layers.")

def train(model, device, train_loader, optimizer, epoch, loss_fn, scaler, 
...
def test(model, device, test_loader, loss_fn):
    """Test loop that computes loss, accuracy, F1 score, MCC, and confusion matrix."""
    model.eval()
    test_loss = 0
    correct = 0
    total_samples = 0
    ...
    return test_loss, accuracy, f1, mcc, conf_matrix

def calculate_class_weights(df):
    """Calculate class weights based on class distribution."""
    class_counts = df['metabolic_state'].value_counts()
    total = len(df)
    # Compute weights as inverse of frequency
    weights = torch.FloatTensor([total / count for count in class_counts])
    # Normalize weights
    weights = weights / weights.sum() * len(weights)
    # Only return weights for the 2 classes we care about
    return weights[:2]

def get_optimizer_and_scheduler(model, train_loader, num_epochs):
    # Use OneCycleLR instead of cosine schedule
    optimizer = Lion(
        model.parameters(),
    ...
    # OneCycleLR often works better than cosine for early plateaus
    scheduler = OneCycleLR(
        ...
        anneal_strategy='cos',
        div_factor=25,  # Start with lower learning rate
        final_div_factor=1e4  # End with very small learning rate
    )
    
    return optimizer, scheduler

# --- run_metabolic_training, incorporate DDP, freeze layers, warmup & mixed precision ---
def run_metabolic_training():
    # Initialize WandB
    wandb.init(
        project="hyena-dna-metabolic-deep-model-classification",
        name="HyenaDNA-Training-Deep-Binary-Classification",
        config={
            "num_epochs": 50,
            "max_length": 4400,
            "batch_size": 64,
    ...
    rc_aug = wandb.config.rc_aug
    use_padding = wandb.config.use_padding
    add_eos = wandb.config.add_eos

    pretrained_model_name = 'hyenadna-metabolic-32k'
    use_head = True
    n_classes = 2  # Binary classification: sensitive vs insensitive

    # --- Distributed setup ---
    if 'RANK' in os.environ:
        setup_distributed()
        local_rank = int(os.environ['LOCAL_RANK'])
        torch.cuda.set_device(local_rank)  # Important: set cuda device before any other cuda calls
    ...
    loss_fn = nn.CrossEntropyLoss(weight=class_weights)  # No need to slice class_weights anymore since calculate_class_weights already returns just 2 weights
    ...
    optimizer, scheduler = get_optimizer_and_scheduler(model, train_loader, num_epochs)
