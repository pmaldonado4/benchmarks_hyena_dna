...
# %%
#df = df.sample(n=100, random_state=42) # used for testing code
import random
import pandas as pd
import numpy as np
...
# Settings
num_classes = 4
sequences_per_class = 250  # Total sequences: 1000
seq_length = 100
motif_length = 10

# Unique motifs for each class
motifs = {
    'fed': 'ATGCGTACGA',
    'fasted': 'CGTACGTAGC',
    'refed': 'GGCATTGGCA',
    'insensitive': 'AAAACCCAAA'
}
...
data = []
for class_name, motif in motifs.items():
    for _ in range(sequences_per_class):
        # Embed motif at a random position
        start_idx = random.randint(0, seq_length - motif_length)
        sequence = random_dna(start_idx) + motif + random_dna(seq_length - start_idx - motif_length)
        data.append((sequence, class_name))

# Convert to DataFrame and shuffle
df = pd.DataFrame(data, columns=['sequence', 'metabolic_state'])
#df = df_synthetic.sample(frac=1).reset_index(drop=True)
...
def test(model, device, test_loader, loss_fn):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            output = model(data)
            loss = loss_fn(output, target.squeeze())
            test_loss += loss.item() * data.size(0)  # multiply by batch_size
            pred = output.argmax(dim=1, keepdim=True)
            correct += pred.eq(target.view_as(pred)).sum().item()
            total_samples += len(data)

    test_loss /= total_samples  # <-- Fix here
    accuracy = 100. * correct / total_samples
...
# Define the deeper classification head on top of Hyena backbone
class HyenaClassifier(nn.Module):
    def __init__(self, backbone, d_model=256, num_classes=4, dropout=0.3):
        super().__init__()
...
# Updated run_metabolic_training function
def run_metabolic_training():
    wandb.init(
        project="hyena-dna-metabolic-metahyena-test",
        name="HyenaDNA-Deeper-Training",
        config={
            "num_epochs": 50,
            "max_length": 110,
            "batch_size": 64,
    ...
    # Wrap backbone with deeper classifier
    model = HyenaClassifier(backbone, d_model=256, num_classes=4, dropout=0.3).to(device)

    tokenizer = CharacterTokenizer(
        characters=['A', 'C', 'G', 'T', 'N'],
        model_max_length=wandb.config.max_length + 2,
        padding_side='left'
    )

    dataset = MetabolicDataset(df=df, max_length=wandb.config.max_length, tokenizer=tokenizer,
                               use_padding=True, rc_aug=True, add_eos=False)
    ...
    model_path = "hyena_dna_deeper_metabolic.pth"
