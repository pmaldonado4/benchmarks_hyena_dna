...
import random as python_random  # Rename random import to avoid conflicts
...
def coin_flip():
    return python_random.random() > 0.5
...
class GenomicBenchmarkDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if content is None:
                raise ValueError(f"Could not read file {txt_path} with any of the encodings: {encodings}")
...
def log_results_to_csv(dataset_name, model_name, accuracy, loss, train_size, test_size, csv_file="benchmark_results.csv"):
    ...
    if not os.path.exists(csv_file):
        with open(csv_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Timestamp', 'Dataset', 'Model', 'Accuracy', 'Loss', 'Random_Seed', 'Train_Size', 'Test_Size'])
...
def main():
    ...
    np.random.seed(42)
    python_random.seed(42)  # Use renamed random module
...
def run_train(device):
    """
    Run training on a single dataset with a specified model.
    """
    try:
