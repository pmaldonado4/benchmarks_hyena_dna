...
# --- Freeze early layers ---
def freeze_early_layers(model, freeze_up_to=2):
    """
...
# %%
def train(model, device, train_loader, optimizer, epoch, loss_fn, scaler, log_interval=10):
    model.train()
    total_loss = 0
    correct = 0
    total_samples = 0

    print(f"\nGPU Memory at start of epoch {epoch}:")
    for i in range(torch.cuda.device_count()):
        memory_allocated = torch.cuda.memory_allocated(i) / 1024**2
        memory_reserved = torch.cuda.memory_reserved(i) / 1024**2
        print(f"GPU {i}: Allocated: {memory_allocated:.1f}MB, Reserved: {memory_reserved:.1f}MB")
...
def run_metabolic_training():
    # Initialize WandB
    wandb.init(
        project="hyena-dna-metabolic-binary-classification",
        name="HyenaDNA-Training-Binary-Classification",
        config={
            "num_epochs": 50,
            "max_length": 4400,
            "batch_size": 64,
